...
int32_t clampClipInstanceLength(int32_t desiredLength, int32_t clipInstancePos, bool hasNextClipInstance,
	...
	int32_t newLength = desiredLength;

	if (hasNextClipInstance) {
		int32_t maxLength = nextClipInstancePos - clipInstancePos;
		if (newLength > maxLength) {
			newLength = maxLength;
		}
	}

	if (newLength > maxSequenceLength - clipInstancePos) {
		newLength = maxSequenceLength - clipInstancePos;
	}
